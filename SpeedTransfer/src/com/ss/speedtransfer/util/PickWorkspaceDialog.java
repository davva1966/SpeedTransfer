...
public class PickWorkspaceDialog extends TitleAreaDialog {
	...
	private static final String _KeyIsRestarting = "wsIsRestarting";
	...
	private static final String _StrMsg = "Your workspace is where your files and settings will be stored.";
	private static final String _StrInfo = "Please select a directory that will be the workspace root";
	private static final String _StrError = "You must select a directory";
	...
	protected Control createDialogArea(Composite parent) {
		...
		try {
			...
			_RememberWorkspaceButton.setToolTipText("You can always switch workspace from inside the application");
		...
		clone.addListener(SWT.Selection, new Listener() {
			...
			public void handleEvent(Event arg0) {
				try {
					...
					// workspace)
					// File wsFile = new File(txt + File.separator +
					// WS_IDENTIFIER);
					File wsFile = new File(txt);
					if (!wsFile.exists()) {
					...
					dd.setText("Select target directory");
					dd.setMessage("Select the directory which will be the target for the clone of workspace " + txt);
					...
					MessageDialog.openError(Display.getDefault().getActiveShell(), "Error", "There was an internal error. Error: " + SSUtil.getMessage(err));
				}
			}
		});
	...
	public static void setIsRestarting(boolean flag) {
		_preferences.putBoolean(_KeyIsRestarting, flag);
	}

	public static boolean isRestarting() {
		return _preferences.getBoolean(_KeyIsRestarting, false);
	}
