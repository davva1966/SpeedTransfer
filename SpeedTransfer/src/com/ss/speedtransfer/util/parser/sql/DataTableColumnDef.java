...
/**
 * The <code>DataTableColumnDef</code> class
 */
public class DataTableColumnDef {
	...
	/**
	 * If this is a foreign key, the table.column that this foreign key refers to.
	 * 
	 * @deprecated
	 */
	...
	public DataTableColumnDef(DataTableColumnDef column_def) {
		System.arraycopy(column_def.constraints_format, 0, constraints_format, 0, constraints_format.length);
	...
	public void setSQLType(int sql_type) {
		...
		if (sql_type == SQLTypes.BIT || sql_type == SQLTypes.BOOLEAN) {
			db_type = Types.DB_BOOLEAN;
		} else if (sql_type == SQLTypes.TINYINT || sql_type == SQLTypes.SMALLINT || sql_type == SQLTypes.INTEGER || sql_type == SQLTypes.BIGINT || sql_type == SQLTypes.FLOAT || sql_type == SQLTypes.REAL || sql_type == SQLTypes.DOUBLE || sql_type == SQLTypes.NUMERIC || sql_type == SQLTypes.DECIMAL) {
			db_type = Types.DB_NUMERIC;
		} else if (sql_type == SQLTypes.CHAR || sql_type == SQLTypes.VARCHAR || sql_type == SQLTypes.LONGVARCHAR) {
			db_type = Types.DB_STRING;
		} else if (sql_type == SQLTypes.DATE || sql_type == SQLTypes.TIME || sql_type == SQLTypes.TIMESTAMP) {
			db_type = Types.DB_TIME;
		} else if (sql_type == SQLTypes.BINARY || sql_type == SQLTypes.VARBINARY || sql_type == SQLTypes.LONGVARBINARY) {
			db_type = Types.DB_BLOB;
		} else if (sql_type == SQLTypes.JAVA_OBJECT) {
			db_type = Types.DB_OBJECT;
		} else {
			db_type = Types.DB_UNKNOWN;
		}
	}
	...
	public void setClassConstraint(String class_constraint) {
		...
		try {
			...
			if (class_constraint.endsWith("[]")) {
				...
				constraining_class = java.lang.reflect.Array.newInstance(ac, 0).getClass();
			} else {
				// Not an array
	...
	/**
	 * Sets this DataTableColumnDef object up from information in the TType object. This is useful when we need to create a DataTableColumnDef object to store information based on nothing more than a
	 * TType object. This comes in useful for purely functional tables.
	 */
	public void setFromTType(TType type) {
		...
		if (type instanceof TStringType) {
			...
			setStringLocale(str_type.getLocaleString(), str_type.getStrength(), str_type.getDecomposition());
	...
	public void initTTypeInfo() {
		if (type == null) {
			type = createTTypeFor(getSQLType(), getSize(), getScale(), getLocaleString(), getStrength(), getDecomposition(), getClassConstraint());
		}
	}
	...
	public String getDBTypeString() {
		switch (getDBType()) {
		case Types.DB_NUMERIC:
			return "DB_NUMERIC";
		case Types.DB_STRING:
			return "DB_STRING";
		case Types.DB_BOOLEAN:
			return "DB_BOOLEAN";
		case Types.DB_TIME:
			return "DB_TIME";
		case Types.DB_BLOB:
			return "DB_BLOB";
		case Types.DB_OBJECT:
			return "DB_OBJECT";
		default:
	...
	public boolean isIndexableType() {
		if (getDBType() == Types.DB_BLOB || getDBType() == Types.DB_OBJECT) {
			return false;
		}
		return true;
	}
