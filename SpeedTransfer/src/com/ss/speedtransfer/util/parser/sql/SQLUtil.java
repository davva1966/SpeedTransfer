...
public class SQLUtil {
	...
	/**
	 * Returns the Token as a non quoted reference. For example, a QUOTED_VARIABLE token will have the first and last '"' character removed. A QUOTED_DELIMINATED_REF will have " removed in each
	 * deliminated section. For example, '"re1"."re2"."a"' becomes 're1.re2.a" and '"re1.re2.a"' becomes 're1.re2.a'.
	 */
	public static String asNonQuotedRef(Token token) {
		...
		} else if (token.kind == SQLParserConstants.QUOTED_DELIMINATED_REF || token.kind == SQLParserConstants.QUOTEDGLOBVARIABLE) {
	...
	/**
	 * Converts a Token which is either a STRING_LITERAL, NUMBER_LITERAL or IDENTIFIER into a Java Object. If 'upper_identifiers' is true then all identifiers are made upper case before being returned
	 * (eg. if the object returns is a Variable object).
	 */
	public static Object toParamObject(Token token, boolean upper_identifiers) {
		...
		// else if (token.kind == SQLParserConstants.NUMBER_LITERAL) {
		// return TObject.bigNumberVal(BigNumber.fromString(token.image));
		// }
		else if (token.kind == SQLParserConstants.BOOLEAN_LITERAL) {
			return TObject.booleanVal(token.image.equalsIgnoreCase("true"));
		} else if (token.kind == SQLParserConstants.NULL_LITERAL) {
			return TObject.nullVal();
		} else if (token.kind == SQLParserConstants.REGEX_LITERAL) {
			// Horrible hack,
			// Get rid of the 'regex' string at the start,
			String str = token.image.substring(5).trim();
			return TObject.stringVal(str);
		} else if (token.kind == SQLParserConstants.QUOTED_VARIABLE || token.kind == SQLParserConstants.GLOBVARIABLE || // eg. Part.*
				token.kind == SQLParserConstants.IDENTIFIER || token.kind == SQLParserConstants.DOT_DELIMINATED_REF || token.kind == SQLParserConstants.QUOTED_DELIMINATED_REF) {
			...
			return v;
		} else if (token.kind == SQLParserConstants.VARIABLE) {
			...
			return new EmbeddedVariable(name);
	 ...
	 * Returns numeric 0
	 */
	public static String xtText(String text) {
	...
	/**
	 * Normalizes the Expression by removing all NOT operators and altering the expression as appropriate. For example, the expression; not ((a + b) = c and c = 5) would be normalized to; (a + b) <>c
	 * or c <>5
	 */
	 ...
	 * or c <>5
	 */
	private static Expression normalize(final Expression exp, final boolean inverse) {
		if (exp.size() <= 1) {
			if (inverse) {
		...
		if (op.isNot()) {
			...
			Expression resolved_expr = new Expression(normalize(exps[0], false), op, normalize(exps[1], false));
			if (inverse) {
			...
			if (inverse) {
				return new Expression(normalize(exps[0], inverse), op.inverse(), normalize(exps[1], inverse));
			} else {
				return new Expression(normalize(exps[0], inverse), op, normalize(exps[1], inverse));

			}
		} else {
			// By this point we can assume the operator is naturally inversible.
			if (inverse) {
				return new Expression(normalize(exps[0], false), op.inverse(), normalize(exps[1], false));
			} else {
				return new Expression(normalize(exps[0], false), op, normalize(exps[1], false));
			}
		}
