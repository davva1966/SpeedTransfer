...
public final class BigNumber extends Number {
	...
	public byte[] toByteArray() {
		if (number_state == 0) {
			return big_decimal.movePointRight(big_decimal.scale()).toBigInteger().toByteArray();
	...
	public int compareTo(BigNumber number) {
		...
		if (number_state == 0) {
			...
			if (number.number_state == 0) {
				...
				if (number.number_state == POS_INF_STATE || number.number_state == NaN_STATE) {
					return -1;
				}
			...
			if (number.number_state == 0) {
				...
				else if (number_state == POS_INF_STATE || number_state == NaN_STATE) {
					return 1;
				} else {
					throw new Error("Unknown number state.");
				}
	...
	/**
	 * The equals comparison uses the BigDecimal 'equals' method to compare values. This means that '0' is NOT equal to '0.0' and '10.0' is NOT equal to '10.00'. Care should be taken when using this
	 * method.
	 */
	...
	public BigNumber bitWiseOr(BigNumber number) {
		if (number_state == 0 && getScale() == 0 && number.number_state == 0 && number.getScale() == 0) {
		...
		if (number_state == 0) {
			if (number.number_state == 0) {
				...
				if (div_by.compareTo(BD_ZERO) != 0) {
					return new BigNumber((byte) 0, big_decimal.divide(div_by, 10, BigDecimal.ROUND_HALF_UP));
				}
			}
		}
	 ...
	 */
	public static final BigNumber NEGATIVE_INFINITY = new BigNumber(NEG_INF_STATE, null);

	public static final BigNumber POSITIVE_INFINITY = new BigNumber(POS_INF_STATE, null);
